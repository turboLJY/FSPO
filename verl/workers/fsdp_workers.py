...
class RewardModelWorker(Worker):
    ...
    def _build_model(self, config):
        ...
        # download the checkpoint from reward_model.model.path
        local_path = copy_local_path_from_hdfs(config.model.path)
        ...
        with init_context(), warnings.catch_warnings():
            ...
            reward_module.to(torch.cuda.current_device())
    ...
    def validate_model_reasoning(self, knowledge, output_str):
        ...
        with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            step_wise_scores = []
            sentence_mask = []
            for sentence in nltk.sent_tokenize(reasoning_str):
                sentence_score = self.reward_module.predict([(knowledge, sentence)])
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            for i in range(len(data)):
                ...
                knowledge = data_item.non_tensor_batch['knowledge']
                ...
                if data_source == "MATH" or data_source == "GSM8K":
                    reasoning_score = 0
                    ...
                    reasoning_score, sentence_mask = self.validate_model_reasoning(knowledge, sequences_str)
                    mask_length = min(len(response_ids), len(sentence_mask))
